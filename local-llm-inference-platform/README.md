...
> Design, build, and operation of an on-premises LLM inference platform on a 2-node NVIDIA DGX Spark (GB10) cluster — multi-backend (SGLang / llama.cpp / vLLM), three distribution strategies, and workload-driven model selection.
>
> NVIDIA DGX Spark（GB10）2ノードクラスタ上に構築した、オンプレミスLLM推論基盤の設計・構築・運用。SGLang / llama.cpp / vLLM のマルチバックエンド構成、3種類の分散方式、ワークロード駆動のモデル選定。
...
An on-premises inference platform for serving open-weight LLMs on sensitive data that must not leave the premises. Two NVIDIA DGX Spark nodes (GB10 — Grace Blackwell, unified memory ~120 GB/node, ARM64) are directly linked with **200 GbE + RoCE v2 (RDMA)** and serve models through **three backends and three distribution strategies**, chosen per workload:

- **SGLang** as the production backend for the high-volume batch pipeline, run as **data-parallel replicas on both nodes** — chosen because the workload (one shared system prompt × similar-format documents) benefits heavily from **RadixAttention's automatic KV-cache reuse**.
- **llama.cpp (CUDA build for `sm_121`)** for development, model validation, and single-shot inference on quantized GGUF models — plus an **RPC-distributed** mode when a single model exceeds one node's memory.
- **vLLM + Ray** with **tensor parallelism across both nodes** for 70B-class models in FP8/safetensors.

**Highlights**
- **Workload-driven backend & topology selection** — a decision framework mapping model size × workload characteristics to the serving configurations, validated with real benchmarks (prefill-bound classification workloads, KV-cache reuse, MoE vs. dense) — culminating in **SGLang for the production batch pipeline**.
...
| 10〜30Bクラスの大量バッチ処理でスループットが欲しい | 各ノードに同一モデルを配置する**データ並列レプリカ**。同一プロンプト×類似文書のワークロードのため、KVキャッシュを自動再利用する**SGLangを本番採用** |
...
1. **マルチバックエンド構成** — SGLang（大量バッチ本番・KVキャッシュ再利用）、llama.cpp（開発・検証・GGUF量子化）、vLLM（FP8/safetensors・大規模TP）をワークロードで使い分け
...
- **「モデル規模 × 要求特性 → 構成」の選択フレームワーク**を確立し、4構成（vLLM TP / llama.cpp単体 / データ並列レプリカ / RPC）を迷わず選べるようにした → [features/02-distributed-inference.md](features/02-distributed-inference.md)
- **ワークロード特性からのバックエンド選定** — 同一プロンプト×類似形式文書の大量処理では**RadixAttentionのKVキャッシュ自動再利用**が支配的と判断し、本番パイプラインに**SGLang**を採用 → [features/01-multi-backend-strategy.md](features/01-multi-backend-strategy.md)
...
| --- | --- | --- |
| 01 | [マルチバックエンド戦略 / Multi-Backend Strategy](features/01-multi-backend-strategy.md) | llama.cpp vs vLLM vs SGLang の実測比較と、KVキャッシュ再利用を決め手にしたSGLang本番採用 |
